...
export const createCommuneSchema = Joi.object({
  ...
  delimitation: Joi.string().allow(null, '').max(100).messages({
    'string.base': 'La délimitation doit être une chaîne de caractères',
    'string.max': 'La délimitation ne doit pas dépasser {#limit} caractères'
  }),
